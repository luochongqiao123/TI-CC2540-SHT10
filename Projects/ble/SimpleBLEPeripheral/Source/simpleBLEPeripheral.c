...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  uint16 VccVoltage = HalAdcRead(HAL_ADC_CHN_VDD3, HAL_ADC_RESOLUTION_14);//¶ÁÈ¡µçÔ´µçÑ¹
  advertData[TEMPHUMI_DATA_LOCATION+6] = (uint8)(VccVoltage>>8);
  advertData[TEMPHUMI_DATA_LOCATION+7] = (uint8)(VccVoltage&0xFF);//¼ÓÈëµ½¹ã²¥Êý¾ÝÖÐ
 /*************************/
}
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if(events & SBP_SHT10_TEMP_EVT){
	...
	osal_mem_free(temp);
       ...
       // AdvertCount++;
        if(AdvertCount++ == 10){ //Ã¿¹ýÒ»¶ÎÊ±¼äÖØÖÃÒ»´Î
          AdvertCount = 0;
...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    ...
    case SIMPLEPROFILE_CHARDEVICENAME:
      {
	uint8* DeviceName_tmp= osal_msg_allocate(sizeof(uint8)*SIMPLEPROFILE_CHARDEVICENAME_LEN);
	osal_memset(DeviceName_tmp, 0, SIMPLEPROFILE_CHARDEVICENAME_LEN);
	SimpleProfile_GetParameter(SIMPLEPROFILE_CHARDEVICENAME, DeviceName_tmp);
	osal_msg_deallocate(DeviceName_tmp);
